...
func HandleNFDiscoveryRequest(request *httpwrapper.Request) *httpwrapper.Response {
	...
	if response != nil {
		...
		return httpwrapper.NewResponse(http.StatusOK, nil, response) // 返回200 ok
	} else if problemDetails != nil {
		return httpwrapper.NewResponse(int(problemDetails.Status), nil, problemDetails) // 根據 problemDetails 的狀態碼返回相應的 HTTP 錯誤狀態碼和問題詳細信息
	}
	// 既沒有有效的 response 也沒有 problemDetails，則它將創建一個包含 HTTP 狀態碼 403 Forbidden 和 "UNSPECIFIED" 原因的 problemDetails，並返回 HTTP 錯誤狀態碼 403 Forbidden
